...
Set-AzureRmContext -SubscriptionId $targetSubscription | Out-Null
...
$key = (Get-AzureRmStorageAccountKey -ResourceGroupName $targetResourceGroupName -name $targetStorageAccount)[0].value
$context = New-AzureStorageContext -StorageAccountName $targetStorageAccount -StorageAccountKey $key
Write-Host "Using storage account: [$targetStorageAccount]" -ForegroundColor Green

# Get available storage containers
$storageContainers = @()
$storageContainers += Get-AzureStorageContainer -Context $context
...
# Get PNG images 
$sourceImages += Get-AzureStorageBlob -Container $targetStorageContainer -Blob "*png" -Context $context

# Get JPG images 
$sourceImages += Get-AzureStorageBlob -Container $targetStorageContainer -Blob "*jp*g" -Context $context
...
foreach($image in $sourceImages)
{
    ...
    if($optimized -eq $true)
    {
		...
		Set-AzureStorageBlobContent -File $imageFilePath -Container $targetStorageContainer -Blob $image.Name -Metadata $metadata -Force -Context $context | Out-Null
    }
